...
when_to_use: When the listing agent runs, or the operator asks to compress a title to 75 characters or build Item Highlights.
allowed-tools:
  - Read
  - Write
  - Glob
...
When run by the listing agent, this is a recommendation section of `reports/listing-audit-<asin>.md`. Full listing rewrites are not required in v1.

If Amazon URL fetch fails, use the listing markdown pack (or catalog CSV) from the connected listings source. Do not fail the artifact. Category unknown after that: infer from the title; if still unknown, say so and apply Home & Kitchen as a conservative default, then note the assumption.

## Inputs

**Single mode:**
- Original title (pasted, or from the listings source)
...
If only a title is pasted with no URL, take category from the listing pack or title before asking.

## Step 1 — Fetch Category from Listing

For each listing:
1. Extract the ASIN from the URL or pack.
2. Fetch the listing page to capture:
...
3. If the page is blocked or returns no breadcrumb, use the markdown pack. Do not stop.
...
Write into `reports/listing-audit-<asin>.md` under **75-character compression**. Do not write a separate client-folder file.
...
This is a recommended compressed title, not a catalog push.

## When to Ask Questions

Ask only if:
- The listing URL is blocked, the markdown pack has no category signal, and the category cannot be inferred from the title
...
Do not ask permission to proceed. Do not block the listing artifact on a failed Amazon fetch.
